...
describe('Тест получения ингредиентов', () => {
  test('Тест успешного получение ингредиентов', async () => {
    ...
    expect(store.getState().ingredients.isLoad).toBe(true);
    ...
    expect(store.getState().ingredients.isLoad).toBe(false);

    //Проверяем, что в стейте лежает те ингредиенты, которые в моке
    expect(store.getState().ingredients.ingredients).toEqual(
      mockIngredients.data
    );
  ...
  test('Неуспешный запрос', async () => {
    ...
    //Проверяем, что в стейте нет ингредиентов
    expect(store.getState().ingredients.ingredients).toEqual([]);
